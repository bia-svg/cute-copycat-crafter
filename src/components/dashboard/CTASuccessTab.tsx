...
import { MousePointer, Eye, TrendingUp, MessageCircle } from "lucide-react";
...
interface WARecord { id: string; clicked_at: string; page_path: string | null }
...
export default function CTASuccessTab() {
  ...
  const [waClicks, setWaClicks] = useState<WARecord[]>([]);
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setWaClicks(data.whatsappClicks || []);
      } catch (e) {
  ...
  const byPage = useMemo(() => {
    const map: Record<string, { views: number; clicks: number; wa: number }> = {};
    views.forEach(v => {
      const key = v.page_path;
      if (!map[key]) map[key] = { views: 0, clicks: 0, wa: 0 };
      map[key].views++;
    });
    clicks.forEach(c => {
  ...
  }, [views, clicks, waClicks]);

  const byDay = useMemo(() => {
    const map: Record<string, { views: number; clicks: number; wa: number }> = {};
    views.forEach(v => {
      const day = format(parseISO(v.viewed_at), "yyyy-MM-dd");
      if (!map[day]) map[day] = { views: 0, clicks: 0, wa: 0 };
      map[day].views++;
    });
    clicks.forEach(c => {
    ...
    waClicks.forEach(w => {
      const day = format(parseISO(w.clicked_at), "yyyy-MM-dd");
      if (!map[day]) map[day] = { views: 0, clicks: 0, wa: 0 };
      map[day].wa++;
    });
    return Object.entries(map)
      .sort((a, b) => a[0].localeCompare(b[0]))
      .map(([date, d]) => ({ date: format(parseISO(date), "dd/MM"), views: d.views, clicks: d.clicks, wa: d.wa }));
  }, [views, clicks, waClicks]);
        ...
        <Card className="bg-white border border-gray-200">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-medium text-gray-500 uppercase">WhatsApp</span>
            </div>
            <div className="text-2xl font-bold">{totalWA}</div>
          </CardContent>
        </Card>
        <Card className="bg-white border border-gray-200">
          <CardContent className="p-4">
      ...
      {byDay.length > 0 && (
        <Card className="bg-white border border-gray-200">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-semibold text-gray-700">Views vs CTA Clicks vs WhatsApp — Daily</CardTitle>
          </CardHeader>
          <CardContent>
            <ChartContainer config={{ views: { label: "Views", color: "#3b82f6" }, clicks: { label: "CTA Clicks", color: "#10b981" }, wa: { label: "WhatsApp", color: "#25D366" } }} className="h-[280px] w-full">
              <BarChart data={byDay}>
                ...
                <Bar dataKey="wa" fill="#25D366" name="WhatsApp" radius={[2, 2, 0, 0]} />
              </BarChart>
            </ChartContainer>
          </CardContent>
        </Card>
              ...
              <TableBody>
                {byPage.map((row) => (
                  <TableRow key={row.path}>
                    ...
                    <TableCell className="text-right">{row.wa}</TableCell>
