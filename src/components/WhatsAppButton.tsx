...
function buildWhatsAppUrl(isEN: boolean): string {
  // Always the same German WhatsApp number, regardless of language/country.
  const phone = "491719539922";
  const utms = getUtmParams();
  const message = isEN
    ? "Hello David, I have a quick question."
    : "Hallo David, ich hätte eine kurze Frage.";
...
export default function WhatsAppButton() {
  ...
  return (
    <a
      href={buildWhatsAppUrl(isEN)}
      target="_blank"
      rel="noopener noreferrer"
      onClick={handleClick}
      className="fixed bottom-7 right-2 z-[39] flex items-center gap-2 bg-[#4CAF50]/90 hover:bg-[#43A047] text-white rounded-full shadow-md hover:shadow-lg transition-all hover:scale-[1.03] md:bottom-5 md:right-5 px-4 py-1.5 md:px-5 md:py-2 backdrop-blur-sm border border-[#1B3A5C]/75"
      aria-label={isEN ? "WhatsApp with David — Free Info" : "WhatsApp mit David — Unverbindliche Info"}
    >
